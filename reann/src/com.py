...
def get_com(coor,force,mass,scalmatrix,numatoms,maxnumatom,table_coor,start_table):
    ...
    if start_table==1 or start_table==5: order_force=np.zeros((ntotpoint,maxnumforce),dtype=scalmatrix.dtype)
    for ipoint in range(ntotpoint):
        ...
        if start_table==1 or start_table==5: order_force[ipoint,0:numatoms[ipoint]*3]=np.array(force[ipoint],dtype=scalmatrix.dtype).reshape(-1)
