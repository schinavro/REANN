...
class ACENNMod(torch.nn.Module):
   def __init__(self,maxnumtype,outputneuron,atomtype,nblock,nl,dropout_p,actfun,initpot=0.0,table_norm=True, cluster_number=None):
      ...
      """
      super(ACENNMod,self).__init__()
      self.register_buffer("initpot",torch.Tensor([initpot]))
      self.cluster_number = cluster_number
      # self.NO, self.norbit = nipsin, * lmax
      ...
      with torch.no_grad():
          ...
          for ele in atomtype:
              modules=[]
              cluster_size = 0
              for clustn in range(1, self.cluster_number+1):
                  cluster_size += nl[0] ** clustn
              linear=Linear(cluster_size, nl[1])
              xavier_uniform_(linear.weight)
              modules.append(linear)
