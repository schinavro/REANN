...
class GetACEDensity(torch.nn.Module):
    def __init__(self,rs,inta,cutoff,neigh_atoms,nipsin,norbit,ocmod_list, ace_kwargs=None):
    ...
    def obtain_orb_coeff(self,iteration:int,totnatom:int,orbital,atom_index12,orb_coeff):
        ...
        if iteration == len(self.ocmod.items()):
            # NTAxNOx(nmax)
            # return torch.square(hyper_worbital)
